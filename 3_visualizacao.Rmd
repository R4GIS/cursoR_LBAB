...
```{r}
library(pacman)
p_load(readr, readxl, dplyr, tidyr, lubridate, ggplot2, ggthemes)
```
...
```{r}
ggplot() +
  geom_line(data = carne_t2, aes(ano, consumo)) +
  geom_line(data = carne_t2, aes(ano, producao))
```
...
ggplot(dados, aes(factor(Idade),
 ...
 labs(x = "Idade (anos)", y = "Materiais genéticos") +
 theme_few() +
 scale_fill_viridis(name = "Altura dominante (m)",
