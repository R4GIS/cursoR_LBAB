...
```{r}
pacman::p_load(readr, dplyr, tidyr, ggplot2, car, agricolae, ExpDes.pt)
```
...
leveneTest(icumsa_leitura ~ tipo + amostra, data = icumsa)
```

```{r}
# analise de variancia

icumsa_anova <-  aov(icumsa ~ tipo * amostra, data = icumsa)
```
